...
class DeviceSetupLampTempViewController: BaseViewController {
    ...
    //    @IBOutlet weak var swMaster: UISwitch!
        ...
        var setupLampTempView: SetupLampTempView?
    ...
    func setUI() {
        ...
        if (setupLampTempView == nil) {
            setupLampTempView = .fromNib()
            setupLampTempView?.actionMaxValue = { (value) in
                self.lblMaxValue!.text = value
            }
            setupLampTempView?.actionMinValue = { (value) in
                self.lblMinValue!.text = value
            }
            setupLampTempView?.setInit(did: m_detailInfo!.m_did, enc: userInfo!.enc, pkMax: pkMax!, pkMin: pkMin!)
            setupLampTempView?.setUI()
        } else {
            setupLampTempView?.setUI()
        }
    ...
    @IBAction func onClick_save(_ sender: UIButton) {
        if (!(setupLampTempView?.vaildCheck() ?? true)) {
            return
        }

        setupLampTempView?.onClick_save()
        
        var _tempmin: Float = setupLampTempView?.m_setMinValue ?? 0.0
        if (UIManager.instance.temperatureUnit == .Celsius) {
            _tempmin = Float(UI_Utility.celsiusToFahrenheit(tempInC: Double(_tempmin)))
        }
        var _tempmax: Float = setupLampTempView?.m_setMaxValue ?? 0.0
        if (UIManager.instance.temperatureUnit == .Celsius) {
            _tempmax = Float(UI_Utility.celsiusToFahrenheit(tempInC: Double(_tempmax)))
        }
        ScreenAnalyticsManager.instance.googleTagManagerCustom(type: .lamp_setting_temperature_range, items: ["lampid_\(m_detailInfo!.m_did)" : "\(_tempmin/100.0)-\(_tempmax/100.0)"])
