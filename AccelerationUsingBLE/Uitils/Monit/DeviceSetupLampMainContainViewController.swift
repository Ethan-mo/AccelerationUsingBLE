//
//  DeviceSetupLampMainTableViewController.swift
//  Monit
//
...
class DeviceSetupLampMainContainViewController: BaseViewController {
    ...
    var m_parent: DeviceSetupLampMainViewController?
    
    var m_originFirmwareXPos: CGFloat = 0
    
    var connectLamp: BleLampInfo? {
        get {
            return DataManager.instance.m_userInfo.connectLamp.getLampByDeviceId(deviceId: m_parent?.m_detailInfo?.m_did ?? 0)
        }
    }
    ...
    func setUI() {
        ...
        setSwAlarmLed(isOn: m_parent!.lampStatusInfo?.isLed ?? false, isAnimation: false)
...
//        UIManager.instance.setContentEnableForConnect(isOn: m_parent!.isConnect, isMaster: true, orginPosX: m_originFirmwareXPos, btnTitle: btnFirmware, lblValue: lblFirmware, imgArrow: nil) // firmware
        ...
        lblDeviceName.text = m_parent!.lampStatusInfo?.m_name ?? ""
        lblApName.text = m_parent!.lampStatusInfo?.m_ap ?? ""
        lblFirmware.text = String(format: "%@ / %@", m_parent!.userInfo?.fwv ?? "", DataManager.instance.m_configData.m_latestLampVersion)
        
        if let _lampStatus = m_parent!.lampStatusInfo {
            let _tempmax = Double(_lampStatus.m_tempmax) / 100.0
            let _tempmaxValue = UIManager.instance.getTemperatureProcessing(value: _tempmax)
            
            let _tempmin = Double(_lampStatus.m_tempmin) / 100.0
            ...
            lblHum.text = "\(_lampStatus.humminValue)% ~ \(_lampStatus.hummaxValue)%"
            lblLed.text = "\(_lampStatus.ledOnTimeStr) ~ \(_lampStatus.ledOffTimeStr)"
        }
        
        imgFirmwareNewAlarm.isHidden = true
        if (m_parent!.isConnect) {
            if (DataManager.instance.m_dataController.newAlarm.lampFirmware.isNewAlarmDetailSetupFirmware(did: m_parent!.m_detailInfo!.m_did)) {
    ...
    func goToScene() {
        if (UIManager.instance.m_finishScenePush == .deviceSetupLampFirmware) {
            ...
            if (m_parent?.isConnect ?? false) {
                let _view = UIManager.instance.sceneMoveNaviPush(scene: .deviceSetupLampFirmware) as! DeviceSetupLampFirmwareViewController
    ...
    @IBAction func onClick_tempUnit(_ sender: UIButton) {
        ...
        NetworkManager.instance.Request(send) { (json) -> () in
            ...
            case .success:
                ...
                ScreenAnalyticsManager.instance.googleTagManagerCustom(type: .lamp_setting_temperature_scale, items: ["lampid_\(self.m_parent!.m_detailInfo!.m_did)" : "\(DataManager.instance.m_userInfo.configData.m_tempUnit)"])
    ...
    func setSwAlarmMasterInfoChange(isOn: Bool) {
        DataManager.instance.m_dataController.userInfo.shareDevice.changeAlarm(did: m_parent!.m_detailInfo!.m_did, type: DEVICE_TYPE.Lamp.rawValue, almType: .all, isOn: isOn)
    }
    
    func isAlarmStatus(almType: ALRAM_TYPE) -> Bool? {
        return DataManager.instance.m_userInfo.shareDevice.isAlarmStatus(did: m_parent!.m_detailInfo!.m_did, type: DEVICE_TYPE.Lamp.rawValue, almType: almType)
    }
    ...
    @IBAction func editingChange_master(_ sender: UISwitch) {
        ...
        ScreenAnalyticsManager.instance.googleTagManagerCustom(type: .lamp_setting_alarm_enabled, items: ["lampid_\(self.m_parent!.m_detailInfo!.m_did)" : "\(sender.isOn ? "true" : "false")"])
    }
    
    @IBAction func onClick_deviceName(_ sender: UIButton) {
        let _vc = UIManager.instance.sceneMoveNaviPush(scene: .deviceSetupLampDeviceName) as? DeviceSetupLampDeviceNameViewController
    ...
    @IBAction func onClick_setAP(_ sender: UIButton) {
        if (connectLamp == nil) {
            ...
            _scene?.setInit(detailInfo: m_parent!.m_detailInfo, connectType: .normal)
            return
        }
        ...
        if let _vc = UIManager.instance.sceneMoveNaviPush(scene: .deviceRegisterLampLight) as? DeviceRegisterLampLightViewController {
            ...
            _vc.m_changeWifiAdvName = m_parent?.userInfo?.adv
            _vc.m_peripheral = connectLamp?.peripheral
        }
    }
    
    @IBAction func onClick_temp(_ sender: UIButton) {
        let _deviceSetupTemp = UIManager.instance.sceneMoveNaviPush(scene: .deivceSetupLampTemp) as! DeviceSetupLampTempViewController
        _deviceSetupTemp.m_detailInfo = m_parent?.m_detailInfo
    }
    
    @IBAction func onClick_hum(_ sender: UIButton) {
        let _deviceSetupHum = UIManager.instance.sceneMoveNaviPush(scene: .deviceSetupLampHum) as! DeviceSetupLampHumViewController
        _deviceSetupHum.m_detailInfo = m_parent?.m_detailInfo
    }
    
    @IBAction func onClick_firmware(_ sender: UIButton) {
        Debug.print("[LampSetup] onClick_firmware", event: .warning)
        let _view = UIManager.instance.sceneMoveNaviPush(scene: .deviceSetupLampFirmware) as! DeviceSetupLampFirmwareViewController
        _view.m_detailInfo = m_parent?.m_detailInfo
    ...
    func confirmDeviceLeave() {
        DataManager.instance.m_dataController.deviceStatus.lampLeave(cid: m_parent!.m_detailInfo!.m_cid, did: m_parent!.m_detailInfo!.m_did, adv: m_parent!.userInfo!.adv)
    }
    
    func confirmDeviceInit() {
        deviceInitProgress() // 서버 에러코드 발생시 중단
        DataManager.instance.m_dataController.deviceStatus.lampInit(did: m_parent!.m_detailInfo!.m_did, enc: m_parent!.userInfo!.enc, adv: m_parent!.userInfo!.adv)
    }
    ...
    func receiveLeaveCloud(_ json: JSON) {
        ...
        switch receive.ecd {
        case .success:
            DataManager.instance.m_userInfo.deviceNoti.deleteItemByDid(type: DEVICE_TYPE.Lamp.rawValue, did: m_parent!.m_detailInfo!.m_did)
            _ = UIManager.instance.sceneMove(scene: .initView, animation: .coverVertical, isAnimation: false)
    ...
    func receiveInitDevice(_ json: JSON) {
        ...
        case .success:
            DataManager.instance.m_userInfo.deviceNoti.deleteItemByDid(type: DEVICE_TYPE.Lamp.rawValue, did: m_parent!.m_detailInfo!.m_did)
            DataManager.instance.m_userInfo.lampGraph.deleteItemByDid(did: m_parent!.m_detailInfo!.m_did)
        default:
    ...
    @IBAction func editingChange_led(_ sender: UISwitch) {
        ...
        ScreenAnalyticsManager.instance.googleTagManagerCustom(type: .lamp_setting_led_indicator_enabled, items: ["lampid_\(m_parent!.m_detailInfo!.m_did)" : "\(sender.isOn ? "true" : "false")"])
    }
    ...
    func setSwAlarmLedInfoChange(isOn: Bool) {
        m_parent!.lampStatusInfo!.isLed = isOn
        ...
        send.onnt = m_parent!.lampStatusInfo!.m_onnt
        send.offt = m_parent!.lampStatusInfo!.m_offt
        NetworkManager.instance.Request(send) { (json) -> () in
    ...
    @IBAction func onClick_settingLed(_ sender: UIButton) {
        let _vc = UIManager.instance.sceneMoveNaviPush(scene: .deivceSetupLampLed) as? DeviceSetupLampLedViewController
        _vc?.m_detailInfo = m_parent!.m_detailInfo
    }
}
