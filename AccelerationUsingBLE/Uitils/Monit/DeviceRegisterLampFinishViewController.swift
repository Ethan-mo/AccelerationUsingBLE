...
class DeviceRegisterLampFinishViewController: BaseViewController {
    ...
    var m_lampStatusInfo: LampStatusInfo?
    var lampStatusInfo: LampStatusInfo? {
        get {
            if let _info = DataManager.instance.m_userInfo.deviceStatus.m_lampStatus.getInfoByDeviceId(did: lampConnectionController?.m_parent?.bleInfo?.m_did ?? 0) {
                return _info
            } else {
                return m_lampStatusInfo
            }
        }
    }
    var lampConnectionController: LampConnectionController?
    var did: Int = 0
    var enc: String = ""
    
    var m_lampNameForm: LabelFormController?
    ...
    override func viewDidLoad() {
        ...
        m_lampStatusInfo = LampStatusInfo(did: lampConnectionController?.m_parent?.bleInfo?.m_did ?? 0, name: "", power: 0, bright: 0, color: 0, attached: 0, temp: 0, hum: 0, voc: 0, ap: "", apse: "", tempmax: 3000, tempmin: 1800, hummax: 6000, hummin: 4000, offt: "0000", onnt: "0000", con: 0, offptime: "", onptime: "")
        ...
        lampConnectionController?.m_parent?.setCloudId()
//        DataManager.instance.m_dataController.deviceStatus.deleteLamp(did: lampConnectionController?.m_parent?.bleInfo?.m_did ?? 0, adv: lampConnectionController?.m_parent?.bleInfo?.m_adv ?? "")
    ...
    func setUI() {
        ...
        lblContentTitle.text = "connection_lamp_connected_title".localized
        if (registerType == .new) {
            lblContentSummary.text = "connection_lamp_connected_detail_with_setting".localized
        } else {
            lblContentSummary.text = "connection_lamp_connected_detail".localized
        }
...
//        btnOtherConnect.isHidden = registerType == .changeWifi
        ...
        if (m_lampNameForm == nil) {
            m_lampNameForm = LabelFormController(txtInput: txtHubName, btnDelete: btnHubNameDelete, minLength: 1, maxLength: 24, imgCheck: imgHubCheckName) // maxLength: Config.MAX_BYTE_LENGTH_NAME
            m_lampNameForm!.setDefaultText(lblDefault: lblHubNameSub, defaultText: "account_hint_baby_name".localized)
            m_lampNameForm!.setDelegate(delegate: NameInfoForm(txtName: txtHubName))
        }
    ...
    func sendHubName() {
        ...
        send.did = lampConnectionController?.m_parent?.bleInfo?.m_did ?? 0
        send.enc = lampConnectionController?.m_parent?.bleInfo?.m_enc ?? ""
        ...
        NetworkManager.instance.Request(send) { (json) -> () in
            ...
            switch receive.ecd {
            case .success:
                self.lampStatusInfo!.m_name = self.txtHubName.text!
            default:
    ...
    func sendTemp() {
        ...
        send.did = lampConnectionController?.m_parent?.bleInfo?.m_did ?? 0
        send.enc = lampConnectionController?.m_parent?.bleInfo?.m_enc ?? ""
        send.tmin = lampStatusInfo!.m_tempmin
        send.tmax = lampStatusInfo!.m_tempmax
        NetworkManager.instance.Request(send) { (json) -> () in
    ...
    func sendHum() {
        ...
        send.did = lampConnectionController?.m_parent?.bleInfo?.m_did ?? 0
        send.enc = lampConnectionController?.m_parent?.bleInfo?.m_enc ?? ""
        send.hmin = lampStatusInfo!.m_hummin
        send.hmax = lampStatusInfo!.m_hummax
        NetworkManager.instance.Request(send) { (json) -> () in
            let receive = Receive_SetAlarmThreshold(json)
            switch receive.ecd {
            case .success: break
    ...
    @IBAction func editing_lampName(_ sender: UITextField) {
        m_lampNameForm?.editing(isTrim: false, isRemoveSpecialChar: true)
    }
    
    @IBAction func onClick_lampNameDelete(_ sender: UIButton) {
        m_lampNameForm?.onClick_delete()
    }
    ...
    @IBAction func editing_temRangeSlider(_ sender: RangeSlider) {
        ...
        lampStatusInfo?.m_tempmin = Int(UIManager.instance.temperatureUnit == .Fahrenheit ? UI_Utility.fahrenheitToCelsius(tempInF: sender.lowerValue) * 100 : sender.lowerValue * 100)
        
        lampStatusInfo?.m_tempmax = Int(UIManager.instance.temperatureUnit == .Fahrenheit ? UI_Utility.fahrenheitToCelsius(tempInF: sender.upperValue) * 100 : sender.upperValue * 100)
    ...
    @IBAction func editing_humRangeSlider(_ sender: RangeSlider) {
        ...
        lampStatusInfo?.m_hummin = Int(sender.lowerValue * 100.0)
        lampStatusInfo?.m_hummax = Int(sender.upperValue * 100.0)
        setRangeSliderUI()
    }
    ...
    @IBAction func onClick_Complete(_ sender: UIButton) {
        ...
        guard (m_lampNameForm!.m_isVaild) else {
        ...
        if (Utility.isUpdateVersion(latestVersion: _lastVer, currentVersion: lampConnectionController?.m_parent?.bleInfo?.m_firmware ?? "9.9.9")) {
            _ = PopupManager.instance.onlyContents(contentsKey: "contents_need_firmware_update", confirmType: .noYes,
                                                   okHandler: { () -> () in
                                                    UIManager.instance.m_moveSceneDeviceType = DEVICE_TYPE.Hub.rawValue
                                                    UIManager.instance.m_moveSceneDeviceID = self.did
                                                    UIManager.instance.setMoveNextScene(finishScenePush: .deviceSetupHubFirmware, moveScene: .initView)
            }, cancleHandler: { () -> () in
                _ = UIManager.instance.sceneMove(scene: .initView, animation: .coverVertical, isAnimation: false)
            })
        } else {
