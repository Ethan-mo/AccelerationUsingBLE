...
class DeviceLampDetailGraphCommonGraphView: UIView, ChartViewDelegate {
    ...
    var m_parent: DeviceLampDetailGraphViewController?
    ...
    var m_filterData: [LampGraphInfo] = []
    ...
    var nowValue: String {
        get {
            ...
            let _lst = DataManager.instance.m_userInfo.lampGraph.m_lst
            for item in _lst {
    ...
    func setUI() {
        ...
        lblDayCurrentTempTitle.text = self.m_state == .tem ? "이 시각의 온도" : "이 시각의 습도"
    ...
    func averageInfo(type: GRAPH_PAGE_TYPE) {
        if (type == .weekly) {
            lblWeeklyNowTempTitle.text = self.m_state == .tem ? "현재 온도" : "현재 습도"
            lblWeeklyNowAvgTempTitle.text = self.m_state == .tem ? "이 날의 평균 온도" : "이 날의 평균 습도"
        } else {
            lblWeeklyNowTempTitle.text = self.m_state == .tem ? "현재 온도" : "현재 습도"
            lblWeeklyNowAvgTempTitle.text = self.m_state == .tem ? "이 달의 평균 온도" : "이 달의 평균 습도"
        }
    ...
    func averageDayInfo(type: GRAPH_PAGE_TYPE, info: LampGraphInfo, value: String) {
        ...
        let _dateSlice = UI_Utility.getDateToSliceDateString(date: _lDate)
        lblDayTitle.text = "\(_dateSlice.1)월 \(_dateSlice.2)일 \(_dateSlice.3)시 \(_dateSlice.4)분"
    ...
    func setDataFilter() {
        let _lst = DataManager.instance.m_userInfo.lampGraph.m_lst
        self.m_filterData = _lst.filter({ (item: LampGraphInfo) -> (Bool) in
...
//        }
    ...
    func reloadNoti() {
        ...
        if (m_lastUpdateTime == nil || m_lastUpdateTime! < NSDate(timeIntervalSinceNow: TimeInterval(-1 * Config.HUB_TYPES_GRAPH_UPDATE_LIMIT)) as Date) {
            if let _detailInfo = m_parent!.m_parent!.m_parent!.m_detailInfo {
                ...
                DataManager.instance.m_dataController.lampGraph.updateByDid(did: _detailInfo.m_did, handler: { (isSuccess) -> () in
                    if (isSuccess) {
    ...
    func getColorByValue(value: Int) -> UIColor {
        ...
        switch m_state {
        case .score:
            switch LampStatusInfo.getScoreType(value: value) {
        ...
        case .tem, .hum, .voc:
            ...
            if let _status = DataManager.instance.m_userInfo.deviceStatus.m_lampStatus.getInfoByDeviceId(did: m_parent!.m_parent!.m_parent!.m_detailInfo?.m_did ?? -1) {
                let _info = LampStatusInfo(did: m_parent!.m_parent!.m_parent!.m_detailInfo?.m_did ?? -1, name: "", power: 0, bright: 0, color: 0, attached: 1, temp: _tempValue, hum: value, voc: value, ap: "", apse: "", tempmax: _status.m_tempmax, tempmin: _status.m_tempmin, hummax: _status.m_hummax, hummin: _status.m_hummin, offt: "", onnt: "", con: 0, offptime: "", onptime: "")
                _retValue = getColorByLampStatusInfo(info: _info)
            }
        }
        return _retValue
    }
    
    func getColorByLampStatusInfo(info: LampStatusInfo?) -> UIColor {
        var _retValue: UIColor = Config.channel == .kc ? COLOR_TYPE.green.color : COLOR_TYPE.blue.color
        if let _info = info {
            let _temp = Double(_info.m_temp) / 100.0
            let _tempValue = UIManager.instance.getTemperatureProcessing(value: _temp)
            ...
            if let _info = item.m_statusInfo {
                ...
                if (_tempValue != -0.01) {
                    ...
                    case .score:
                        ...
                        _chartYColors.append(getColorByLampStatusInfo(info: _info))
