...
class DeviceSetupHubLedViewController: BaseViewController, UIPickerViewDataSource,UIPickerViewDelegate {
    ...
    override var screenType: SCREEN_TYPE { get { return .HUB_SETUP_LED } }
    ...
    var userInfo: UserInfoDevice? {
     ...
     override func viewWillAppear(_ animated: Bool) {
      ...
      func setUI() {
          ...
          //        if let _almStatus = isAlarmStatus(almType: .hubTemp) {
          //            setSwMaster(isOn: _almStatus, isAnimation: false)
          //        }
      }
    ...
    @IBAction func onClick_save(_ sender: UIButton) {
        if let _statusInfo = hubStatusInfo {
            ...
            NetworkManager.instance.Request(send) { (json) -> () in
                ...
                switch receive.ecd {
                case .success:
                    ScreenAnalyticsManager.instance.googleTagManagerCustom(type: .hub_setting_led_indicator_enabled_time, items: ["hubid_\(self.m_detailInfo!.m_did)" : "\(self.m_setMinValue)-\(self.m_setMaxValue)"])
