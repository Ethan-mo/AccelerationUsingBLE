...
class DeviceSetupLampConnectingViewController: BaseViewController {
    ...
    var connectLamp: BleLampInfo? {
        get {
            return DataManager.instance.m_userInfo.connectLamp.getLampByDeviceId(deviceId: m_detailInfo?.m_did ?? 0)
        }
    }
    ...
    func setInit(detailInfo: DeviceDetailInfo?, connectType: BLE_CONNECT_TYPE) {
        Debug.print("[LampSetupFirmwareConnecting] setInit()")
        m_detailInfo = detailInfo
        m_connectType = connectType
    }
    
    func setUI() {
        Debug.print("[LampSetupFirmwareConnecting] setUI()")
...
//        lblConnectingTitle.text = "guide_direct_connection_for_firmware_update".localized
    ...
    @objc func update() {
        ...
        if (UIManager.instance.rootCurrentView as? DeviceSetupLampConnectingViewController == nil) {
            self.m_updateTimer?.invalidate()
        }
